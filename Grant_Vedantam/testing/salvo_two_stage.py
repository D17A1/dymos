...
def solve_vehicle(initial_phi, initial_psi, final_theta, final_phi, final_gamma, final_psi,
    ...
    v_sim     = sim.get_val('traj.phase0.timeseries.v').flatten()
    gamma_sim = sim.get_val('traj.phase0.timeseries.gamma').flatten()
    psi_sim   = sim.get_val('traj.phase0.timeseries.psi').flatten()
...
###############################################################################
# Two-stage (re-planning / MPC-like) workflow
...
A1_state = get_state_at_time(case_A1['sim'], t_reveal)
...
B1_state = get_state_at_time(case_B1['sim'], t_reveal)
...
print_vehicle_state_before_impact(case_A, dt_before=10.0)
...
plt.plot(tA, case_A['v'], label='Vehicle A')
plt.plot(tB, case_B['v'], label='Vehicle B')
plt.xlabel('Time (s)')
plt.ylabel('Velocity')
plt.grid(True)
plt.legend()
